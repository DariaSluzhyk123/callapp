...
<body>
    ...
    <section class = "blok1">
        ...
        <?php
    $zapytanie1 ="SELECT id, imie, nazwisko FROM `maturzysta` WHERE szkola = 'T3' order by nazwisko;";
    $wynik1 = mysqli_query($conn, $zapytanie1);
    while ($row = mysqli_fetch_row($wynik1)) {
        echo "<p><a href='wynik.php?id=$row[0]&imie=$row[1]&nazwisko=$row[2]'>$row[0] $row[1] $row[2]</a></p>";
    }
    ?>
    </section>
    <section class = "blok2">
